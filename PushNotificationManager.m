...
- (void) writeDeviceID:(NSString *) deviceId {
    ...
    genericPasswordQuery[(__bridge id)kSecAttrService] = ident;
    genericPasswordQuery[(__bridge id)kSecAttrAccount] = ident;
    
    genericPasswordQuery[(__bridge id)kSecAttrAccessible] = (__bridge id)kSecAttrAccessibleAlwaysThisDeviceOnly;
    genericPasswordQuery[(__bridge id)kSecValueData] = [deviceId dataUsingEncoding:NSUTF8StringEncoding];
...
- (NSString *) readDeviceId {
    ...
    genericPasswordQuery[(__bridge id)kSecReturnData] = (__bridge id)kCFBooleanTrue;
...
- (instancetype) initWithApplicationCode:(NSString *)_appCode appName:(NSString *)_appName{
    if(self = [super init]) {
        ...
        (self.locationTracker).locationUpdatedInForeground = ^ (CLLocation *location) {
            if (!location)
                return;
        ...
        (self.locationTracker).locationUpdatedInBackground = ^ (CLLocation *location) {
            if (!location)
                return;
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    NSDictionary *lastPushDict = pushNotifications[[NSNumber numberWithInt:alertView.tag]];
...
- (BOOL) handlePushReceived:(NSDictionary *)userInfo {
    ...
    if(!pushDict) {
        ...
        userInfo = userInfo[UIApplicationLaunchOptionsRemoteNotificationKey];
        pushDict = userInfo[@"aps"];
        isPushOnStart = YES;
    }
    ...
    NSString *alertMsg = pushDict[@"alert"];
    ...
    if(!isPushOnStart && showPushnotificationAlert && msgIsString) {
        ...
        pushNotifications[[NSNumber numberWithInt:internalIndex]] = userInfo;
        [alert show];
        return YES;
    }
