...
-(void) getImages
{
    ...
    if (sqlite3_open(databasepath.UTF8String, &dbSecret) == SQLITE_OK) {
        ...
        NSString *sql = [NSString stringWithFormat:@"select * from ViewImageLogtbl  where  isLogin = \"true\" AND UserID=%@ Order BY imageID DESC",app.LoginUserID];
        NSLog(@"query is %@",sql);
