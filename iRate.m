...
- (NSString *)messageTitle
{
    ...
    return _messageTitle ? : [self localizedStringForKey:iRateMessageTitleKey withDefault:@"SHOW SOME LOVE!"];
}
...
- (NSString *)message
{
    ...
    {
#ifdef LITEVERSION
        message = (self.appStoreGenreID == iRateAppStoreGameGenreID)? [self localizedStringForKey:iRateGameMessageKey withDefault:@"If you are enjoying Secret Vault - Private Albums Manager, give it a 5 star rating!\nThanks for the supporting this app!"]: [self localizedStringForKey:iRateAppMessageKey withDefault:@"If you are enjoying Secret Vault - Private Albums Manager, give it a 5 star rating!\nThanks for the supporting this app!"];
#else
       message = (self.appStoreGenreID == iRateAppStoreGameGenreID)? [self localizedStringForKey:iRateGameMessageKey withDefault:@"If you are enjoying Secret Vault Pro - Private Albums Manager, give it a 5 star rating!\nThanks for the supporting this app!"]: [self localizedStringForKey:iRateAppMessageKey withDefault:@"If you are enjoying Secret Vault Pro- Private Albums Manager, give it a 5 star rating!\nThanks for the supporting this app!"]; 
#endif
    }
...
{
    return _rateButtonLabel ?: [self localizedStringForKey:iRateRateButtonKey withDefault:@"Rate ★ ★ ★ ★ ★"];
}
...
- (NSUInteger)usesCount
{
    ...
    launchCount=[[NSUserDefaults standardUserDefaults] integerForKey:iRateUseCountKey];
    NSLog(@"%lu",(unsigned long)launchCount);
    return launchCount;
}
    ...
    if(self.usesCount > 3)
    {
        if (!self.visibleAlert)
        {
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:self.messageTitle
                                                            message:@"Give Secret Vault a rating in the AppStore!"
                                                           delegate:(id <UIAlertViewDelegate>)self
                                                  cancelButtonTitle:@"No Love"
                                                  otherButtonTitles:@"LOVE IT", nil];
#else
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:self.messageTitle
                                                            message:@"Give Secret Vault Pro a rating in the AppStore!"
                                                           delegate:(id <UIAlertViewDelegate>)self
                                                  cancelButtonTitle:@"No Love"
                                                  otherButtonTitles:@"LOVE IT", nil];
#endif
    ...
    else
    {
        ...
        if (![self.delegate respondsToSelector:@selector(iRateShouldOpenAppStore)] || [_delegate iRateShouldOpenAppStore])
        {
            //go to ratings page
             ...
             [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://itunes.apple.com/WebObjects/MZStore.woa/wa/viewContentsUserReviews?id=569771443&pageNumber=0&sortOrdering=1&type=Purple+Software&mt=8"]];
